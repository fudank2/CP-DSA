...
bool checksum(int arr[],int val,int n)
{
    if(val==0)return true;
    else if(n==0&&val!=0)return false;

    if(val>=arr[n-1])
    {
        return checksum(arr,val-arr[n-1],n-1)||checksum(arr,val,n-1);
    }
    else{
        return checksum(arr,val,n-1);
    }
}
...
void solve()
{
    ...
    int val;cin>>val;
    rep(i,0,val+1)dp[0][i]=0;
    rep(i,0,n+1)dp[i][0]=0;
    cout<<checksum(arr,val,n);
}
