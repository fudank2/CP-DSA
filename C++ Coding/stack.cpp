...
// in this file i'm going to do array implementation of a stack
...
class Stack{
    ...
    bool isempty()
    {
        if(top_in==-1)return true;
        else false;
    }
    int top()
    {
        if(top_in==-1)return-1;
        else return a[top_in];
    }
    void push(int val)
    {
        if(top_in==5000)cout<<"Stack is empty"<<"\n";
        top_in++;
        a[top_in]=val;
    }
    void pop()
    {
        if(top_in==-1)cout<<"Stack is empty"<<"\n";
        top_in--;
    }
};
