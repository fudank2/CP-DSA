...
bool c1=0;
void checkstring(string s,string p,int i,int j,int sn,int pn)
{
    if(i==sn&&j==pn&&(s[i]==p[j]||p[j]=='*'||p[j]=='?')){c1=1;return;}
    else if(j==pn&&p[j]=='*')c1=1;
    else if(s[i]!=p[j]&&(p[j]!='?'&&p[j]!='*')||i>sn||j>pn){}
    else if(s[i]==p[j]||p[j]=='?'&&i<sn&&j<pn) checkstring(s,p,i+1,j+1,sn,pn);
    else if(p[j]=='*')
    {
        for(int k=i;k<=sn;k++)
        {
            while(p[j]=='*'&&j<pn)j++;
            checkstring(s,p,k,j,sn,pn);
        }
    }

}

bool stringmatch(string s,string p,int n,int m)
{
    if(m==0)return (n==0);

    bool dp[n+1][m+1]={0};
    dp[0][0]=1;
    for(int i=1;i<=m;i++)if(p[i-1]=='*')dp[0][i]=1;
...
void solve()
{
    ...
    int sn=s.size();
    int pn=p.size();
    //checkstring(s,p,0,0,sn-1,pn-1);
    cout<<stringmatch(s,p,sn,pn);
    //cout<<c1<<"\n";
}
